...
#' #' @export
#' li.lme.score2 <- function(subjectData, beta, sigma0, sigma1, rho, sigmae){
#'     yi          <- subjectData[["yi"]]
#'     xi          <- subjectData[["xi"]]
#'     zi          <- subjectData[["zi"]]
#'     t.zi        <- t(zi)
...
#'     subjectData <- CreateSubjectData(id=id,y=y,x=x,z=z,Weights=Weights,SampProb=SampProb,cutpoints=cutpoints,w.function=w.function)
#'
#'     UncorrectedScorei <- lapply(subjectData, li.lme.score2, beta=beta, sigma0=sigma0, sigma1=sigma1, rho=rho, sigmae=sigmae)
...
#'     z      = model.matrix(formula.random, rand.f)
#'
...
#'     }
#'
#'     AllNames <- c("(Intercept)", attr(fixed.t,"term.labels"), "log(sig.0)","log(sig.1)","FisherZ(rho)","log(sig.e)")
#'     out              <- NULL
#'     out$call         <- match.call()
#'     out$coefficients <- acml.fit$estimate
#'     out$covariance   <- solve(ObsInfo)
#'     out$robcov       <- solve(ObsInfo)%*%Cheese%*%solve(ObsInfo)
#'     names(out$coefficients) <- rownames(out$covariance) <- rownames(out$robcov) <- colnames(out$covariance) <- colnames(out$robcov) <- AllNames
...
#' #    out$n.iter       <- acml.fit$iterations
#'
